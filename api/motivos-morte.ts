/**
 * API route for motivos de morte (motivos de morte de animais).
 *
 *   GET    ?organizationId=xxx              — list motivos for org (auto-seed defaults if empty)
 *   POST   { organizationId, nome, descricao? }         — create
 *   POST   { action: 'reorder', items: [{id, ordem}] } — reorder
 *   PATCH  { id, nome?, descricao? }                    — update
...
import {
  ...
  remove,
  reorder,
} from '../src/DB/repositories/motivos-morte.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.error('[motivos-morte] error:', err);
