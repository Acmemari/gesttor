/**
 * API route para Movimentação › Nascimento.
 *
 *   GET    ?organizationId=xxx   — lista movimentos de nascimento da org (com fichas)
 *   GET    ?id=xxx               — busca um movimento pelo id (com fichas)
 *   POST   { organizationId, farmId?, localId?, proprietarioId?, data, safra?,
 *            retiro?, qtd, naoIdentificados, status, catDecl[], sanitario[], fichas[] }
 *                                — cria movimento + fichas individuais
 *   POST   { action:'add-ficha', movimentoId, apelido, categoriaId?, rfid?, sisbov?,
 *            porte?, raca?, peso? }
...
import {
  ...
  addFicha,
  deleteMovimento,
} from '../src/DB/repositories/nascimentos.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'add-ficha') {
        const { movimentoId, apelido, categoriaId, rfid, sisbov, porte, raca, peso } = body;
        if (!movimentoId || !apelido) {
          jsonError(res, 'Campos obrigatórios: movimentoId, apelido', { status: 400 });
          return;
        }
        const updated = await addFicha(movimentoId, {
          ...
          porte: porte || null,
          raca: raca || null,
          peso: peso != null && peso !== '' ? toNum(peso) : null,
        });
        if (!updated) {
      ...
      const {
        ...
        qtd, naoIdentificados, status, catDecl, sanitario, fichas,
      } = body;
      ...
      const row = await createMovimento({
        ...
        catDecl: Array.isArray(catDecl)
          ? catDecl.map((c: any) => ({ catId: String(c.catId), qtd: Math.max(0, Math.trunc(toNum(c.qtd))) }))
          : [],
        sanitario: Array.isArray(sanitario) ? sanitario : [],
    ...
    console.error('[nascimentos] error:', err);
    jsonError(res, 'Erro ao processar nascimentos. Tente novamente.', { code: 'INTERNAL', status: 500 });
  }
}
