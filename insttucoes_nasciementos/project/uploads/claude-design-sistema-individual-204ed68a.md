...
- Lançamento rápido primeiro (poucos campos + botões de ação); identificação individual opcional, vem depois.
...
### 4.1. Padrão de botões em TODA tela de movimentação (incl. Nascimento)

Todo formulário de movimentação tem **dois botões**, lado a lado, no rodapé do card:

- **Novo** (secundário, contorno azul) — limpa o formulário e o deixa pronto para um novo lançamento em branco (zera data para hoje, quantidade para 1, campos individuais vazios). Não salva nada.
- **Salvar** (primário, azul cheio) — grava o movimento, move o estoque e dispara o toast. Após salvar, mantém o usuário na tela com o formulário já limpo para o próximo lançamento (mesmo efeito do "Novo"), agilizando lançamentos em série no curral.

> Use o rótulo **"Salvar"** (ou "Salvar e mover estoque" como variação aceitável). O botão **"Novo"** vem sempre à esquerda do "Salvar".

### 4.2. Tela de Nascimento — campos obrigatórios

A tela **Lançar Nascimento** deve conter:

- Data · Responsável · Categoria (com saldo) · Quantidade (cabeças) · Lote de destino.
- **Campo "ID do animal" / "Brinco do bezerro" (opcional)** — onde o usuário digita o identificador individual do bezerro nascido (brinco, RFID ou ID interno). Texto de apoio: *"Deixe vazio se o bezerro ainda não foi identificado — ele recebe um ID interno automático e pode ser brincado depois."*
- Quando o ID é preenchido, cria a ficha individual já vinculada ao nascimento; quando vazio, o animal entra como "não identificado" (estado normal), contando na quantidade do estoque e gerando pendência de identificação na Mesa apenas se a quantidade individualizada for menor que a quantidade do estoque.
- Rodapé com os botões **Novo** e **Salvar** (conforme 4.1).
