...
/* ===== LOTES ===== */
function viewLotes(){
  const finCls = {Recria:'blue',Engorda:'green',Reprodução:'',Uniformidade:''};
  const rows = DB.lotes.map(l=>{
    ...
    return `<tr>
      ...
      <td class="sub-cell">${l.sistema}</td>
  ...
  <div class="panel">
    ...
    ${panelNote('O lote guarda apenas <b>código, finalidade, sistema produtivo e status</b>. A população do lote é o saldo derivado das alocações — não se digita animal no cadastro do lote.')}
    <table class="tbl">
      <thead><tr><th>Código</th><th>Lote</th><th>Finalidade</th><th>Sistema produtivo</th><th>Status</th><th>População (derivada)</th><th></th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </div>`;
}
function openNovoLote(id){
  ...
  openModal({title:l?'Editar lote':'Novo lote',sub:'Identidade do lote. Sem local, sem lista de animais — isso é derivado.',body:`<div class="form-grid">
    ...
    <div class="field"><label>Finalidade</label><select id="lo-fin">${['Recria','Engorda','Reprodução','Uniformidade'].map(f=>`<option ${l&&l.finalidade===f?'selected':''}>${f}</option>`).join('')}</select></div>
    <div class="field"><label>Sistema produtivo</label><input id="lo-sis" value="${l?l.sistema:''}" placeholder="Pasto, Confinamento…"></div>
...
function salvarLote(id){
  const codigo=val('lo-cod'),nome=val('lo-nome'),finalidade=val('lo-fin'),sistema=val('lo-sis'),status=val('lo-st');
  if(!codigo){toast({title:'Informe o código',kind:'crit'});return;}
  if(id){Object.assign(DB.lotes.find(x=>x.id===id),{codigo,nome,finalidade,sistema,status});}
  else DB.lotes.push({id:'lote-'+Date.now(),codigo,nome,finalidade,sistema,status});
...
function viewFicha(){
  ...
  return pageHead({
    ...
    actions:`<button class="btn primary" onclick="openNovoAnimal()">${icon('plus')} Nova ficha</button>`,
  }) + `
...
function openFicha(id){
  ...
  const hist = DB.movimentos.filter(m=>m.vinculados.includes(id)).sort((x,y)=>y.data.localeCompare(x.data));
  const pes = DB.pesagens.filter(p=>p.animal===id).sort((x,y)=>x.data.localeCompare(y.data));
  const gmd = gmdAnimal(id);
  const lote = loteAtualAnimal(id);
  const histHtml = hist.length ? hist.map(m=>`<div class="kv"><span class="k">${fmtData(m.data)} · ${tipoLabel(m.tipo)}</span><span class="v">${m.loteDestino?nomeLote(m.loteDestino):'—'}</span></div>`).join('') : '<div class="small" style="color:var(--muted)">Sem movimentos vinculados.</div>';
  const pesHtml = pes.length ? pes.map(p=>`<div class="kv"><span class="k">${fmtData(p.data)}</span><span class="v">${p.peso} kg</span></div>`).join('') : '<div class="small" style="color:var(--muted)">Sem pesagens.</div>';
  openModal({
    title:`Ficha ${a.id}`,
    sub:`${a.sexo} · ${a.raca} · ${nomeCategoria(a.categoria)}`,
    body:`
      <div class="form-grid" style="margin-bottom:6px">
        <div class="aside-card"><h4>Identidade</h4>
          <div class="kv"><span class="k">ID interno (chave)</span><span class="v mono" style="color:var(--blue)">${a.id}</span></div>
          <div class="kv"><span class="k">Brinco</span><span class="v">${a.brinco?`<span class="mono">${a.brinco}</span>`:'<span class="pill pend"><span class="d"></span>Sem id.</span>'}</span></div>
          <div class="kv"><span class="k">Status do brinco</span><span class="v">${a.statusBrinco}</span></div>
          <div class="kv"><span class="k">Lote atual (derivado)</span><span class="v">${lote?nomeLote(lote):'—'}</span></div>
        </div>
        <div class="aside-card"><h4>Desempenho</h4>
          ${gmd?`<div class="kv"><span class="k">GMD</span><span class="v" style="color:var(--ok)">${gmd.gmd.toFixed(3)} kg/dia</span></div>
          <div class="kv"><span class="k">Peso atual</span><span class="v">${gmd.pesoFim} kg</span></div>
          <div class="kv"><span class="k">Período</span><span class="v">${Math.round(gmd.dias)} dias · ${gmd.n} pesagens</span></div>`:'<div class="small" style="color:var(--muted)">Sem dados de GMD.</div>'}
        </div>
      </div>
      <div class="aside-card" style="margin-bottom:14px"><h4>Histórico de movimentos</h4>${histHtml}</div>
      <div class="aside-card"><h4>Curva de peso</h4>${pesHtml}</div>
    `,
    foot:`<div class="spacer"></div><button class="btn" data-close>Fechar</button>`,
  });
}
function openNovoAnimal(){
  openModal({title:'Nova ficha individual',sub:'Só o ID interno é obrigatório. O brinco é opcional — sem ele, o animal entra como "não identificado".',body:`<div class="form-grid">
    <div class="field"><label>ID interno</label><input id="an-id" placeholder="A-0011"></div>
    <div class="field"><label>Brinco <span class="opt">(opcional)</span></label><input id="an-br" placeholder="—"></div>
    <div class="field"><label>Sexo</label><select id="an-sexo"><option>Macho</option><option>Fêmea</option></select></div>
    <div class="field"><label>Raça</label><input id="an-raca" placeholder="Nelore"></div>
    <div class="field full"><label>Categoria</label><select id="an-cat">${DB.categorias.map(c=>`<option value="${c.id}">${c.nome}</option>`).join('')}</select></div>
  </div>`,foot:`<div class="spacer"></div><button class="btn ghost" data-close>Cancelar</button><button class="btn primary" onclick="salvarAnimal()">${icon('save')} Salvar ficha</button>`});
}
function salvarAnimal(){
  const id=val('an-id'),br=val('an-br'),sexo=val('an-sexo'),raca=val('an-raca'),categoria=val('an-cat');
  if(!id){toast({title:'Informe o ID interno',kind:'crit'});return;}
  DB.animais.push({id,brinco:br||null,statusBrinco:br?'ok':'sem',sexo,raca,categoria,vivo:true});
  closeModal();render();
