...
const tabs = [
  ...
  { key: 'transformations', label: 'Transformações', icon: Target },
...
export const ProjectModal: React.FC<ProjectModalProps> = ({ form, onChange, onSave, onClose, saving, mode, people = [] }) => {
  ...
  >
    {/* Tabs */}
    <div className="flex gap-1 border-b border-ai-border -mx-6 px-6">
      {tabs.map(tab => {
        const Icon = tab.icon;
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors -mb-px ${
              isActive
                ...
                : 'border-transparent text-ai-subtext hover:text-ai-text hover:border-ai-border'
            }`}
          >
            <Icon size={15} />
            {tab.label}
    ...
    {/* Tab 1 — Informações Básicas + Cronograma */}
    {activeTab === 'info' && (
    ...
    {/* Tab 2 — Transformações e Evidências */}
    {activeTab === 'transformations' && (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-xl bg-blue-50/60 border border-blue-100 p-4">
            ...
            {form.transformations.map((transformation, tIdx) => (
              <div
                ...
                className={`flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer transition-colors ${
                  tIdx === effectiveIdx
                    ? 'border-ai-accent bg-white'
                    : 'border-ai-border bg-white/60 hover:border-ai-accent/40'
                }`}
              ...
              {selectedTrans.evidence.map((ev, eIdx) => (
                <div key={`ev-${effectiveIdx}-${eIdx}`} className="flex items-center gap-2">
                  <input
                    ...
                    className="w-full rounded-lg border border-ai-border bg-white px-3 py-2 text-sm text-ai-text placeholder:text-ai-subtext/50"
                  />
                  <button
                    type="button"
                    onClick={() =>
