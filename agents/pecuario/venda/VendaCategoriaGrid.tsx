...
import type { VendaCat } from './types';

interface VendaCategoriaGridProps {
  rows: VendaCat[];
...
/**
 * Grade das categorias (lotes) da Venda Abate: cada linha é uma categoria com a
 * quebra Sem ID / Com ID / Total de cabeças (Com ID = detalhamento individual,
 * ainda em breve). Editar/Remover agem sobre a linha local (chave `id`). Menu •••
 * ancorado por coordenadas e renderizado em portal no body, para escapar do
 * overflow-hidden e do container do cartão.
 */
const VendaCategoriaGrid: React.FC<VendaCategoriaGridProps> = ({ rows, onEdit, onRemove }) => {
  ...
  // Limita a 4 linhas visíveis; a partir da 5ª, a tabela rola (cabeçalho fixo).
...
export default VendaCategoriaGrid;
