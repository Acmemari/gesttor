...
import { formatDateBR, computeVendaAbate, fmtNum, fmtPct, fmtBRL } from './util';
import type { MovimentoVenda } from './types';

interface VendaLancamentosRecentesProps {
  movimentos: MovimentoVenda[];
...
/** Resumo "Categoria (qtd), ..." a partir dos itens da venda. */
function catSummary(m: MovimentoVenda, catName: (id: string) => string): string {
...
/**
 * Relação dos lançamentos de venda (master-detail). Cada linha abre/fecha para
 * mostrar os itens por categoria e o bloco de valores (informados + calculados).
 * Ações por linha: Ver, Editar, Excluir.
 */
const VendaLancamentosRecentes: React.FC<VendaLancamentosRecentesProps> = ({
  movimentos,
  catName,
  pessoaName,
  onEditar,
  ...
  if (!movimentos.length) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-gray-200 bg-white py-16 text-center">
        ...
        <h4 className="text-sm font-bold text-gray-700">Nenhuma venda registrada ainda</h4>
        <p className="max-w-sm text-[12.5px] leading-relaxed text-gray-400">
          Vá até a aba “Lançamentos” para registrar a primeira venda do rebanho.
        </p>
      </div>
    );
  }
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {movimentos.map((m) => {
            ...
            const calc = computeVendaAbate(m.itens, m.tipoVenda, m.tipoPeso, m.desconto);
              ...
              <React.Fragment key={m.id}>
                ...
                >
                  ...
                  <td className="border-b border-[#f1f2f4] px-4 py-3 text-right tabular-nums text-gray-700">
                    {/* Peso por cabeça na unidade da venda: kg (Pé ou Abate kg) ou @ (Abate arroba). */}
                    {m.tipoPeso === 'kg'
                      ? (calc.pesoMortoCab != null ? `${fmtNum(calc.pesoMortoCab, 0)} kg` : '—')
                      : (calc.pesoMortoArroba != null ? `${fmtNum(calc.pesoMortoArroba, 1)} @` : '—')}
                  </td>
                  <td className="border-b border-[#f1f2f4] px-4 py-3 text-right font-semibold tabular-nums text-[#16a34a]">{fmtBRL(calc.valorTotal)}</td>
                  ...
                  <tr>
                    <td colSpan={8} className="border-b border-[#f1f2f4] bg-[#fbfcfd] px-6 py-4">
                      ...
                      <div className="mb-3 flex flex-wrap items-center gap-2 text-[11.5px]">
                        ...
                        {m.cliente ? (
                          <span className="inline-flex items-center rounded-full bg-[#e7f6ec] px-2.5 py-1 font-semibold text-[#16a34a]">
                            Cliente: {pessoaName(m.cliente)}
                          </span>
                        ) : null}
                      </div>

                      {/* Bloco de valores (informados + calculados) */}
                      {m.tipoVenda === 'pe' ? (
                        ...
                        <div className={`mb-3 grid grid-cols-2 gap-x-6 gap-y-3 rounded-lg border border-gray-200 bg-white p-3.5 sm:grid-cols-4 ${m.tipoPeso === 'kg' ? 'lg:grid-cols-6' : 'lg:grid-cols-7'}`}>
                          <ValorItem label={m.tipoPeso === 'kg' ? 'Valor/kg médio' : 'Valor/@ médio'} value={fmtBRL(calc.valorArroba)} />
                          <ValorItem label="Peso morto" value={calc.totalPesoMorto > 0 ? `${fmtNum(calc.totalPesoMorto, 0)} kg` : '—'} />
                          <ValorItem label="Peso morto/cab" value={calc.pesoMortoCab != null ? `${fmtNum(calc.pesoMortoCab, 1)} kg` : '—'} />
                          {m.tipoPeso === 'kg' ? null : (
                            <ValorItem label="Peso morto @" value={calc.pesoMortoArroba != null ? `${fmtNum(calc.pesoMortoArroba, 2)} @` : '—'} />
                          )}
                          <ValorItem label="Rendimento" value={fmtPct(calc.rendimento)} />
                          <ValorItem label="Valor/cab" value={fmtBRL(calc.valorCab)} />
                          <ValorItem label="Valor total" value={fmtBRL(calc.valorTotal)} strong />
                        </div>
                      )}
                            ...
                            <table className="w-full border-collapse text-[12.5px]">
                              <thead>
                                <tr className="text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400">
                                  ...
                                  <th className="border-b border-gray-100 px-3 py-2 text-right">
                                    {m.tipoPeso === 'kg' ? 'Valor/kg' : 'Valor/@'}
                                  </th>
                                  <th className="border-b border-gray-100 px-3 py-2 text-right">
                                    {m.tipoVenda === 'pe' ? 'Peso vivo total' : 'Peso morto'}
                                  </th>
                                  {m.tipoVenda === 'pe' && (
                                    <>
                                      <th className="border-b border-gray-100 px-3 py-2 text-right">Desconto</th>
                                      <th className="border-b border-gray-100 px-3 py-2 text-right text-[#16a34a]">Peso Líquido</th>
                                    </>
                                  )}
                                </tr>
                              </thead>
                                    ...
                                    <tr key={it.id}>
                                      ...
                                      {m.tipoVenda === 'pe' && (
                                        <>
                                          ...
                                          <td className="border-b border-[#f4f5f6] px-3 py-2 text-right tabular-nums text-[#16a34a] font-semibold">
...
export default VendaLancamentosRecentes;
