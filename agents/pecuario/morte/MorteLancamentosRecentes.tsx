...
import { ChevronDown, ChevronRight, Eye, FilePen, IdCard, MoreHorizontal, Trash2, Skull, Tag, Cpu, Plus, Check, Info } from 'lucide-react';
import { createPortal } from 'react-dom';
import { formatDateBR } from './util';
import type { LookupItem, MovimentoMorte } from './types';
import type { MotivoMorte } from '../../../lib/api/motivosMorteClient';

/** Ficha de atribuição de ID emitida pelo formulário inline. */
export interface AtribMorteFicha {
  idManejo: string;
  idEletronico: string;
  catId: string;
  motivoId: string;
...
interface MorteLancamentosRecentesProps {
  movimentos: MovimentoMorte[];
  categories: LookupItem[];
  motivos: MotivoMorte[];
  catName: (id: string) => string;
  motivoName: (id: string) => string;
  /** individualiza uma baixa coletiva pendente (atribuição de ID) */
  onAddFicha: (movId: string, ficha: AtribMorteFicha) => void;
...
/**
 * Resumo da distribuição por categoria do movimento (ex.: "Bezerra mamando (2),
 * Vaca (1)") — mesmo padrão da tela de Nascimento. Consolida os animais
 * identificados (fichas, por catId) com as baixas coletivas declaradas (catDecl).
 */
function catSummary(m: MovimentoMorte, catName: (id: string) => string): string {
 ...
 * pendente (informa ID de Manejo/Eletrônico, categoria e motivo). Mantém estado
 * próprio para resetar a cada animal adicionado.
 */
const AtribuirForm: React.FC<{
  ...
  onAddFicha: (movId: string, ficha: AtribMorteFicha) => void;
  onToast?: (msg: string, type: 'success' | 'error' | 'warning' | 'info') => void;
}> = ({ movId, categories, motivos, restantes, onAddFicha, onToast }) => {
  ...
  const add = () => {
    ...
    onAddFicha(movId, { idManejo: manejo, idEletronico: eletronico, catId, motivoId, obs: obs.trim() });
    setIdManejo('');
    setIdEletronico('');
    setObs('');
  };
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-end gap-2.5">
        {/* ID de Manejo — preenche um, carrega o outro (mesmo esquema da tela de Lançamentos) */}
        ...
        <div className="min-w-[130px] flex-1">
          ...
          <select className={`${inputCls} mt-1.5`} value={motivoId} onChange={(e) => setMotivoId(e.target.value)}>
            <option value="">Selecione</option>
            {motivos.map((m) => (
...
/**
 * Relação dos lançamentos de morte (master-detail). Cada linha abre/fecha para
 * mostrar os animais detalhados, as baixas coletivas e — quando há baixa
 * pendente — o formulário de Atribuir ID. Ações por linha: Ver, Editar,
 * Atribuir ID e Excluir (paridade com a tela de Nascimento).
 */
const MorteLancamentosRecentes: React.FC<MorteLancamentosRecentesProps> = ({
  movimentos,
  categories,
  motivos,
  catName,
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-gray-200 bg-white py-16 text-center">
        ...
        <p className="max-w-sm text-[12.5px] leading-relaxed text-gray-400">
          Vá até a aba “Lançamentos” para registrar a primeira morte do rebanho.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
                            ...
                            <table className="w-full border-collapse text-[12.5px]">
                              <thead>
                                <tr className="text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400">
                                  ...
                                  <th className="border-b border-gray-100 px-3 py-2">Motivo</th>
                              ...
                              <tbody>
                                {m.fichas.map((f) => (
                                  <tr key={f.id}>
                                    ...
                                    <td className="border-b border-[#f4f5f6] px-3 py-2 text-gray-600">{motivoName(f.motivoId)}</td>
                              ...
                              <tbody>
                                {m.catDecl.map((c, i) => (
                                  <tr key={`${c.catId}-${i}`}>
                                    ...
                                    <td className="border-b border-[#f4f5f6] px-3 py-2 text-gray-600">{c.motivoId ? motivoName(c.motivoId) : '—'}</td>
      ...
      {/* Menu de ações (•••) — paridade com Nascimento: Ver, Editar, Atribuir ID, Excluir */}
      {menu
        ? createPortal(
            <>
...
export default MorteLancamentosRecentes;
